...
play.clickMan = function (key,x,y){
	...
	if (play.nowManKey&&play.nowManKey != key && man.my != com.mans[play.nowManKey ].my){
		...
		if (play.indexOfPs(com.mans[play.nowManKey].ps,[x,y])){
			...
			socket.emit("mankey",{key:play.nowManKey,pace:`${com.mans[play.nowManKey].x},${com.mans[play.nowManKey].y},${x},${y}`,deleteKey:key});
...
play.clickPoint = function (x,y){
	...
	if (play.nowManKey){
		if (play.indexOfPs(com.mans[key].ps,[x,y])){
			...
			//console.log(man.x,man.y,x,y);
			socket.emit("mankey",{key:key,pace:`${man.x},${man.y},${x},${y}`});
			//z(bill.createMove(play.map,man.x,man.y,x,y))
...
window.addEventListener("load", ()=>{
 ...
 socket.on(`mankey_${DELI.parameterURL("room")}`,(res)=>{
	if(res.uid != DELI.parameterURL("uid")){
		var pace = res.pace.split(",");
		var x1 = 8 - parseInt(pace[0]);
		var y1 = 9 - parseInt(pace[1]);
		var x2 = 8 - parseInt(pace[2]);
		var y2 = 9 - parseInt(pace[3]);
 ...
 socket.on(`joiner_ready_${DELI.parameterURL("room")}`,(res)=>{
	var ready = true;
	document.getElementById(`joiner_${res.uid}`).getElementsByTagName("span")[0].innerHTML = "Ready";
	var joiner = document.getElementsByClassName("joiner");
	for(let i=0; i<joiner.length; i++){
		if(joiner[i].getElementsByTagName("span")[0].innerText != "Ready"){				
			ready = false;
		}
	}
	...
	if (ready){			
		...
		play.depth = 3;
		play.init();		
		if(res.firstJoiner[0].creator == DELI.parameterURL("uid")){
			setTimeout(()=>{		
				play.isPlay = true;
			},3000);		
		}	
